...
struct bsdsum_test {
	...
	{ TEST_RESULT_STR, STYLE_TERSE, "SIZE", "", "0", },
	{ TEST_RESULT_STR, STYLE_TERSE, "SIZE", "abc", "3", },
	{ TEST_RESULT_STR, STYLE_BASE64, 
		"MD5", "abc", "kAFQmDzST7DWlj99KOF/cg==", },
	{ TEST_PARSE, STYLE_NONE, "PARSE1", "", NULL, NULL },
	{ TEST_PARSE, STYLE_UNSUPPORTED, "PARSE2", 
...
void bsdsum_autotest(void)
{
	...
	for (i = 0; tests[i].test; i++) 
	{
		...
		if (tests[i].type == TEST_RESULT_HEX ||
			...
			hf = bsdsum_find_alg (tests[i].name, 0, 1);
			bsdsum_digest_init(hf, -1);
			...
			st = bsdsum_parse(s, &filename, &dg, &hf);
			if ((st != tests[i].style) ||
				(hf && 
